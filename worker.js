...
async function handleAuth(request, env) {
  ...
  if (!env.ADMIN_PASSWORD) {
    return json({ ok: false, error: 'ADMIN_PASSWORD is not configured in Cloudflare' }, 500);
  }
...
async function handleContent(request, env) {
  ...
  if (request.method === 'POST') {
    ...
    return json({
      ...
      verified: !!verify,
      servicesCount: Array.isArray(savedContent.services) ? savedContent.services.length : 0
    });
  }
